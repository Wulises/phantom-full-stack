...
export default function FunctionsJs() {
  ...
  const [num3, setNum3] = useState<number | ''>(4)
  ...
  const handleRest = (): void => {
    const numerosValidos = [num1, num2, num3].filter((n) => n !== '')
    if (numerosValidos.length === 0) {
      setResultadoRest(null)
      return
    }
    const resultado = sumaRest(...(numerosValidos as number[]))
    setResultadoRest(resultado)
  }
      ...
      <div className="space-y-6">
        ...
        <div className="bg-black/30 border-l-4 border-HMorado p-6 rounded-xl shadow-inner space-y-4">
          ...
          <p className="text-white/80 text-sm italic">
            Ingresa hasta tres números para ver cómo los parámetros <code>rest</code> se agrupan.
          </p>
          ...
          <div className="grid grid-cols-3 gap-4">
            <input
              ...
              className="w-full p-2 rounded-md bg-black/80 border border-white/20 text-white"
              value={num1}
              onChange={(e) => handleNumeroChange(setNum1, e.target.value)}
            />
            <input
              type="number"
              className="w-full p-2 rounded-md bg-black/80 border border-white/20 text-white"
              value={num2}
              onChange={(e) => handleNumeroChange(setNum2, e.target.value)}
            />
            <input
              type="number"
              className="w-full p-2 rounded-md bg-black/80 border border-white/20 text-white"
              value={num3}
              onChange={(e) => handleNumeroChange(setNum3, e.target.value)}
            />
          </div>
